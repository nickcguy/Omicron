...
{
    // Do not resize if in the mixed reality mode
...
void OculusWorldDemoApp::OnKey(OVR::KeyCode key, int chr, bool down, int modifiers)
{
    ...
    case Key_F6:
        if (!down)
        {
            ...
            if (IsMixedRealityMode)
                IsMixedRealityMode = SetupMixedReality();
            if (!IsMixedRealityMode)
                CleanupMixedReality();
        }
        return;
...
// Apparently one of the build targets in the build machine is non-private
    ...
    // as much by queue ahead logic.
    {
        ...
        pRender->SetDefaultRenderTarget();
        pRender->Clear(0.0f, 1.0f, 0.0f, 0.0f);	// green screen for mixed reality mode
...
{
    ovrResult error = ovr_GetExternalCameras(Session, &ExternalCameras[0], &NumberOfCameras);
    // qxdebug
...
void OculusWorldDemoApp::CleanupMixedReality()
{
    NumberOfCameras = 0;
    CurrentCameraID = -1;
