...
#define TracePhaseSyncBegin(                                                     \
    Frame, SecBeforeSemaphore, SecBeforeSleep, SecBeforeSpin, SecPhaseSyncDelay) \
...
#define TracePhaseSyncEnd(                                                                 \
  ...
  if (EventEnabledPhaseSyncEnd()) {                                                        \
    uint32_t framesMissed =                                                                \
        (uint32_t)((float)((VSyncSec) - (LastVSyncSec)) / (Frame).FrameInterval - 0.5f);   \
    float syncFrameTime = (SyncQueueAheadSec) + (Frame).FrameInterval;                     \
    float syncQueueAhead = (SyncQueueAheadSec) / (Frame).FrameInterval;                    \
    uint32_t syncFramesMissed = (uint32_t)(                                                \
        ((FrameSec) - (SyncQueueAheadSec) + (SyncDistortionSec)) / (Frame).FrameInterval); \
